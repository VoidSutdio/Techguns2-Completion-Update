...
public class RenderDungeonScanner extends TileEntitySpecialRenderer<DungeonScannerTileEnt> {
    ...
    @Override
    public void render(@NotNull DungeonScannerTileEnt te, double x, double y, double z, float partialTicks, int destroyStage,
                       float alpha) {
    ...
    private void drawGhost(SegmentType type, int sizeXZ, int sizeY) {
        ...
        //draw ceiling
        if (type != SegmentType.RAMP && type != SegmentType.FOUNDATION && type != SegmentType.PILLARS)
            drawGhostBox(offset, offset + sizeY - 1, offset, sizeXZ - 2 * offset, 1.0 - 2 * offset, sizeXZ - 2 * offset, 0.5f, 1.0f, 0.5f, 0.25f);
    ...
    protected void drawGhostBoxPlane(BufferBuilder buffer, double x, double y, double z, double w_x, double h, double w_z, float r, float g, float b, float alpha) {
        buffer.pos(x, y + h, z + w_z).tex(0, 1).color(r, g, b, alpha).endVertex();
        buffer.pos(x + w_x, y + h, z).tex(1, 1).color(r, g, b, alpha).endVertex();
        buffer.pos(x + w_x, y, z).tex(1, 0).color(r, g, b, alpha).endVertex();
        buffer.pos(x, y, z + w_z).tex(0, 0).color(r, g, b, alpha).endVertex();
    }
}
