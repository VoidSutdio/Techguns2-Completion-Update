...
public class RenderOreDrill extends TileEntitySpecialRenderer<OreDrillTileEntMaster> {
    ...
    protected ResourceLocation texture = new ResourceLocation(Tags.MOD_ID, "textures/blocks/drillhead.png");
    protected OreDrillCube tessellatorcube = new OreDrillCube(texture);

    protected ResourceLocation texture2 = new ResourceLocation(Tags.MOD_ID, "textures/blocks/drillhead_obsidian.png");
    private final OreDrillCube tessellatorcube2 = new OreDrillCube(texture2);

    protected ResourceLocation texture3 = new ResourceLocation(Tags.MOD_ID, "textures/blocks/drillhead_carbon.png");
    private final OreDrillCube tessellatorcube3 = new OreDrillCube(texture3);

    @Override
    public void render(@NotNull OreDrillTileEntMaster te, double x, double y, double z, float partialTick, int destroyStage,
                       float alpha) {
        //super.render(te, x, y, z, partialTick, destroyStage, alpha); //Renders floating name text

        if (te.isFormed()) {

            EnumFacing drill_direction = te.getDrill_direction();

            int[] vec = getNextBlockInDirOffset(0, 0, 0, drill_direction, te.getEngines() + 1);
            ...
            GL11.glPushMatrix();
            int rot = (int) (te.getWorld().getTotalWorldTime() % 360);
            ...
            GL11.glTranslatef(0, 2 * (te.getEngines() + 1), 0);

            if (te.hasDrill() && te.isRedstoneEnabled() && te.hasPower()) {
            ...
            if (te.hasDrill()) {
                if (te.getDrillType() == 1) {
                    drawDriller(te, tessellatorcube, tess);
                } else if (te.getDrillType() == 2) {
                    drawDriller(te, tessellatorcube2, tess);
                } else if (te.getDrillType() == 3) {
                    drawDriller(te, tessellatorcube3, tess);
                }
            } else {
                drawDrillRod(te, tess);
            }
    ...
    protected void drawDriller(OreDrillTileEntMaster tile, OreDrillCube cube, Tessellator tess) {
        ...
        for (int i = 0; i < length * DRILLERCUBESPERBLOCK; i++) {
            ...
            cube.drawCube(axis[0] == 0 ? size : height, (axis[1] == 0 ? size : height), (axis[2] == 0 ? size : height), tess);
    ...
    protected void drawDrillRod(OreDrillTileEntMaster tile, Tessellator tess) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            tessellatorcube.drawCube(axis[0] == 0 ? size : height, (axis[1] == 0 ? size : height), (axis[2] == 0 ? size : height), tess);
