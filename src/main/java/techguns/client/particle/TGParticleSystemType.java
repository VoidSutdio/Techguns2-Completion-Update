...
public class TGParticleSystemType extends TGFXType {
    ...
    public static IVelocityType VEL_NONE = (sys, params) -> new Vec3d(0, 0, 0);

    public static IVelocityType VEL_ORTHO = (sys, params) -> {
    ...
    public static IVelocityType VEL_OUTWARD = (sys, params) -> {
        Random rand = new Random();
    ...
    public static IVelocityType VEL_SPHERICAL = (sys, params) -> {
        Random rand = new Random();
...
//			}
    ...
    public static IVelocityType VEL_HEMISPHERICAL = (sys, params) -> {
        Random rand = new Random();
        if (sys.type.velocityDataMin.length >= 1) {
            float r = MathUtil.randomFloat(rand, sys.type.velocityDataMin[0], sys.type.velocityDataMax[0]);
...
//				}
    ...
    public static IVolumeType VOL_POINT = (sys, dir, i, count) -> new Vec3d(0, 0, 0);


    public static IVolumeType VOL_CYLINDER = (sys, dir, i, count) -> {
        Random rand = new Random();
        if (sys.type.volumeData.length >= 2) {
            ...
            return new Vec3d(direction.x * r, y, direction.z * r);
        } else {
            //fail;
            return new Vec3d(0, 0, 0);
        }
    ...
    //Tilted cylinder
    public static IVolumeType VOL_CYLINDER2 = (sys, dir, i, count) -> {
        Random rand = new Random();
        if (sys.type.volumeData.length >= 2) {
            float r;
            float z;
    ...
    public static IVolumeType VOL_SPHERE = (sys, dir, i, count) -> {
        Random rand = new Random();
    ...
    public static IVolumeType VOL_HEMISPHERE = (sys, dir, i, count) -> {
        Random rand = new Random();
        if (sys.type.volumeData.length >= 1) {
            float r;
            double angle = rand.nextDouble() * 2.0 * Math.PI;
    ...
    public static IVolumeType VOL_TRAIL = (sys, dir, i, count) -> {
        if (sys.type.volumeData.length >= 1) {
    ...
    public interface IVelocityType {
        Vec3d getVelocity(TGParticleSystem sys, float... params);
    }

    public interface IVolumeType {
        Vec3d getPosition(TGParticleSystem sys, DirResult dir, int i, int count);
    }
    ...
    public void extend(TGParticleSystemType other) {
        ...
        System.arraycopy(other.volumeData, 0, this.volumeData, 0, other.volumeData.length);
