...
public class MazeDungeonPath implements IDungeonPath {
            ...
            if (isWithinBounds(nextPos)) {
                if (dir == nextDir && !segment.isEntrance && !fork && rand.nextFloat() < chanceRamp) { //ramp
                    ...
                    if (rand.nextFloat() < chanceRoom) {//Try to place a Room
                        ...
                        if (!b) { //just a regular segment it is
    ...
    public static class PathSegment {
        ...
        public void printPattern() {
            ...
            char t = '1';
            char f = '0';
            if (pattern[0]) sb.append(t);
            else sb.append(f);
            if (pattern[1]) sb.append(t);
            else sb.append(f);
            if (pattern[2]) sb.append(t);
            else sb.append(f);
            sb.append("\n");
            if (pattern[7]) sb.append(t);
            else sb.append(f);
            sb.append(" ");
            if (pattern[3]) sb.append(t);
            else sb.append(f);
            sb.append("\n");
            if (pattern[6]) sb.append(t);
            else sb.append(f);
            if (pattern[5]) sb.append(t);
            else sb.append(f);
            if (pattern[4]) sb.append(t);
            else sb.append(f);
                ...
                for (int k = 0; k < sZ; k++) {
                    ...
                    if (segment != null) {
                        ...
                        if (segment.isRamp) {
                            if (segment.elevation == 1) {
                                ...
                                if (useRoof && !above && segment.y + 1 < this.sY) {
                                    ...
                                    preset.getSegment(SegmentType.END, -1, 0, this.sY, false, below, seed).placeSegment(world, px, py, pz, (r + 1) % 4);
                                }
                            }
                        } else {
                            ...
                            for (TemplateSegment tempSeg : TemplateSegment.templateSegments.values()) {
                                ...
                                if (match) {
                                    ...
                                    if (useRoof && !above && segment.y >= startHeightLevel && segment.y + 1 < this.sY) {
                                        ...
                                        int seed = segment.roomID > 0 ? segment.roomID : world.rand.nextInt();
                                        preset.getSegment(tempSeg.type, -1, 0, this.sY, false, below, seed).placeSegment(world, px, py, pz, r);
                                    }
                                    break;
                                }
                            }
                            if (!ok) {
                                System.out.println("couldn't match segment " + segment);
                                segment.printPattern();
                            }
                        }
                    } else {
                        if (useFoundations && above && !below) {
                            ...
                            int py = posY + (j * preset.getSizeY());
                            int pz = posZ + (k * preset.getSizeXZ());
                            preset.getSegment(SegmentType.FOUNDATION, j, 0, this.sY, true, false, world.rand.nextInt()).placeSegment(world, px, py, pz, 0);
                        } else if (usePillars && above) {
                            int px = posX + (i * preset.getSizeXZ());
                            int py = posY + (j * preset.getSizeY());
                            int pz = posZ + (k * preset.getSizeXZ());
                            preset.getSegment(SegmentType.PILLARS, j, 0, this.sY, true, below, world.rand.nextInt()).placeSegment(world, px, py, pz, 0);
    ...
    public void generateNPCSpawners(World world, int posX, int posY, int posZ, IDungeonPreset preset) {
        ...
        for (PathSegment seg : spawnPositions) {
            ...
            int py = posY + (seg.y * preset.getSizeY()) + preset.getSpawnYOffset();
            ...
            if (pos.getY() >= 1) {
                ...
                if (te instanceof TGSpawnerTileEnt spawner) {
    ...
    private List<PathSegment> getSpawnPositions(float spawnDensity) {
        ...
        List<PathSegment> spawners = new ArrayList<>();
