...
public class IngredientHasNBTTag extends Ingredient {
    ...
    public ItemStack @NotNull [] getMatchingStacks() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (matchingStacks[i].getItem() instanceof GenericGun g) {
    ...
    protected static Object getTagByType(byte id, NBTTagCompound tags, String key) {
        return switch (id) {
            case 1 -> tags.getByte(key);
            case 2 -> tags.getShort(key);
            case 3 -> tags.getInteger(key);
            case 4 -> tags.getLong(key);
            case 5 -> tags.getFloat(key);
            case 6 -> tags.getDouble(key);
            case 7 -> tags.getByteArray(key);
            case 8 -> tags.getString(key);
            case 10 -> tags.getCompoundTag(key);
            case 11 -> tags.getIntArray(key);
            default -> null;
        };
    }
