...
public class GenericGunMeleeCharge extends GenericGunCharge implements IGenericGunMelee<GenericGunMeleeCharge> {
    ...
    public void setMiningHeads(ItemStack... heads) {
    ...
    public void setHasCustomAnim(boolean customAnim) {
    ...
    @Override
    public boolean onBlockStartBreak(@NotNull ItemStack itemstack, @NotNull BlockPos pos, EntityPlayer player) {
            ...
            if (sideHit != null) {
                ...
                otherblocks.forEach(b -> {
                    if (w.isBlockLoaded(b)) {
                        ...
                        if (!(state.getBlock() == Blocks.AIR) && state.getPlayerRelativeBlockHardness(player, w, b) > 0f
    ...
    public boolean setGunStat(EnumGunStat stat, float value) {
        ...
        if (stat == EnumGunStat.MINING_SPEED) {
            this.digSpeed = value;
            return true;
        }
        return false;
