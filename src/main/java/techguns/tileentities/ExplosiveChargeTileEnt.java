...
public class ExplosiveChargeTileEnt extends BasicOwnedTileEnt implements ITickable {
    ...
    public void update() {
        if (this.armed) {
            ...
            if (!this.world.isRemote && this.fusetime > 0 && (this.fusetime % 20 == 0)) {
                this.world.playSound(null, this.pos, getTickSound(), SoundCategory.BLOCKS, 4.0F, 1.0f);
            }
    ...
    protected void explode() {
        ...
        if (!this.world.isRemote) {
            this.world.playSound(null, this.pos, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F);
            ...
            this.getPos().getAllInBoxMutable(pos.offset(side1, rad).offset(side2, rad), pos.offset(side1.getOpposite(), rad).offset(side2.getOpposite(), rad)).forEach(p -> explodeLine(p, facing));
