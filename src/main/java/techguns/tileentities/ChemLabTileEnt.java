...
public class ChemLabTileEnt extends BasicMachineTileEnt implements ITileEntityFluidTanks {
    ...
    public ChemLabTileEnt() {
        ...
        this.inventory = new ItemStackHandlerPlus(5) {
            ...
            protected boolean allowItemInSlot(int slot, ItemStack stack) {
                return switch (slot) {
                    case SLOT_BOTTLE -> ChemLabRecipes.allowInFlaskSlot(stack);
                    case SLOT_INPUT1, SLOT_INPUT2 -> isItemValidForSlot(slot, stack);
                    case SLOT_UPGRADE -> TGItems.isMachineUpgrade(stack) || TGItems.isSteamUpgrade(stack);
                    default -> false;
                };
            }
    ...
    public ITextComponent getDisplayName() {
        return new TextComponentTranslation(Tags.MOD_ID + ".container.chemlab");
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack item) {
        ...
        } else if (slot == SLOT_INPUT1) {
            if (this.input2.get().isEmpty()) return ChemLabRecipes.isItemValidForSlot1(item);
            else return ChemLabRecipes.isRecipeMatch(item, this.input2.get());
        } else if (slot == SLOT_INPUT2) {
            if (this.input1.get().isEmpty()) return ChemLabRecipes.isItemValidForSlot2(item);
            else return ChemLabRecipes.isRecipeMatch(this.input1.get(), item);
        }
        return false;
    }
    ...
    public int getValidSlotForItemInMachine(ItemStack item) {
        ...
        boolean validFor1 = ChemLabRecipes.isItemValidForSlot1(item);
        boolean validFor2 = ChemLabRecipes.isItemValidForSlot2(item);
        ...
        if (this.input1.get().isEmpty() && validFor1) {
            if (!this.input2.get().isEmpty()) {
                if (ChemLabRecipes.isRecipeMatch(item, this.input2.get())) return SLOT_INPUT1;
            } else {
                return SLOT_INPUT1;
            }
        }

        if (this.input2.get().isEmpty() && validFor2) {
            if (!this.input1.get().isEmpty()) {
                if (ChemLabRecipes.isRecipeMatch(this.input1.get(), item)) return SLOT_INPUT2;
            } else {
                return SLOT_INPUT2;
            }
        }

        if (TGItems.isMachineUpgrade(item) || TGItems.isSteamUpgrade(item)) {
            return SLOT_UPGRADE;
        }
        return -1;
    }
    ...
    public static class ChemLabFluidHandler implements IFluidHandler {
        ...
        private final ChemLabTileEnt tile;
