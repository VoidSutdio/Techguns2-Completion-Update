...
public class OreDrillTileEntMaster extends MultiBlockMachineTileEntMaster implements ITileEntityFluidTanks {
    ...
    public ITextComponent getDisplayName() {
        return new TextComponentTranslation(Tags.MOD_ID + ".container.oredrill");
    }
    ...
    protected boolean consumePower(int amount) {
        ...
        if (this.fuelBuffer * FACTOR >= amount) {
            this.fuelBuffer -= (int) Math.max(amount / FACTOR, 1);
            return true;
        } else {
            amount -= (int) (this.fuelBuffer * FACTOR);
            this.fuelBuffer = 0;
    ...
    public int getClusterSize(int count, BlockPos startPos, ArrayList<BlockPos> visited, IBlockState clusterMeta) {
        ...
        if (targetState == clusterMeta && !visited.contains(startPos)) {
            ...
            count = getClusterSize(count, startPos.add(1, 0, 0), visited, clusterMeta);
            count = getClusterSize(count, startPos.add(-1, 0, 0), visited, clusterMeta);
            count = getClusterSize(count, startPos.add(0, 1, 0), visited, clusterMeta);
            count = getClusterSize(count, startPos.add(0, -1, 0), visited, clusterMeta);
            count = getClusterSize(count, startPos.add(0, 0, 1), visited, clusterMeta);
            count = getClusterSize(count, startPos.add(0, 0, -1), visited, clusterMeta);
    ...
    public static class OreDrillFluidHandler implements IFluidHandler {
        ...
        private final OreDrillTileEntMaster tile;
