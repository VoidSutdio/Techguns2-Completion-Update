...
public class TurretTileEnt extends BasicPoweredTileEnt implements ITickable {
    ...
    public ITextComponent getDisplayName() {
        return new TextComponentTranslation(Tags.MOD_ID + ".container.turretbase");
    }
            ...
            for (int i = 0; i < emptyMag.length; i++) {
                ...
                if (InventoryUtil.consumeAmmo(this.inventory, ammo[i], SLOT_INPUT1, SLOT_INPUT1 + INPUTS_SIZE)) {
                    if (!emptyMag[i].isEmpty()) {
                        int tooMuch = InventoryUtil.addItemToInventory(this.inventory, new ItemStack(emptyMag[i].getItem(), 1, emptyMag[i].getItemDamage()), SLOT_OUTPUT1, SLOT_OUTPUT1 + OUTPUTS_SIZE);
                        if (!this.world.isRemote) {
                            this.world.spawnEntity(new EntityItem(this.world, this.pos.getX(), this.pos.getY() + 1, this.pos.getZ(), new ItemStack(emptyMag[i].getItem(), tooMuch, emptyMag[i].getItemDamage())));
                        }
                    }
    ...
    public void spawnTurret(World w) {
